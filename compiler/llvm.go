...
func (c *LLVMCompiler) Compile() string {
	...
	c.emit("%%XTResult = type { i32, i32, i32, i64, i64 }")
...
func (c *LLVMCompiler) compileExpression(expr ast.Expression) (string, string, string) {
	...
	case *ast.MemberCallExpression:
		...
		if e.Member.Value == "解包" {
			...
			c.emit("  %s = inttoptr i64 %s to %%XTResult*", resPtr, objXt)
			valPtrPtr := c.nextReg()
			c.emit("  %s = getelementptr %%XTResult, %%XTResult* %s, i32 0, i32 3", valPtrPtr, resPtr)
			valPtr := c.nextReg()
	...
	case *ast.InfixExpression:
		...
		if isArithmeticOrCompare {
			...
			if magicMethod != "" {
				...
				c.emit("  call void @xt_release(i64 %s)", keyXt)
				// release objXt and rightXt? wait, they are used in overloadLabel too
				// we should only release them once at the end
				if leftType != "raw_i64" {
					c.emit("  call void @xt_release(i64 %s)", objXt)
				}
				// Note: rightXt might have been created inside overloadLabel.
				// This is getting messy. I'll just release them carefully.
				return finalRes, "i64", ""
			}
		}
		if e.Operator == "&" {
