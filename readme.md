...
[![版本](https://img.shields.io/badge/版本-0.16.0-red.svg)](https://gitee.com/mark-jy/xuantie)
...
> 目前玄铁的开发团队由 1 人组成，即作者。作者负责设计、实现、测试和维护玄铁。AI辅助开发则负责生成代码、优化代码、修复错误等。

玄铁由中国广东一名 17 岁开发者独立设计并实现。从 2026 年 4 月 3 日的 v0.2 起步，在一个月内已迭代至 v0.16.0，目前正处于**自举编译器 (Self-Hosted Compiler)** 的核心攻坚阶段。
...
2. **第二步：渲染引擎**  
   实现基于 `raylib` 或类似底层封装的玄铁原生渲染引擎，提供 `画`、`界面` 等原语。
3. **第三步：玄铁铸造厂 (IDE)**  
